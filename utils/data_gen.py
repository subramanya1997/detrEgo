...
def gen_or_load_dataset(config):
    ...
    if config.suffix is None:
        save_path = os.path.join(config.save_dir, f"dataset_{config.num_querys}_{config.query_fmodel}_{config.video_seq_len}.pt")
    else:
        save_path = os.path.join(config.save_dir, f"dataset_{config.num_querys}_{config.query_fmodel}_{config.video_seq_len}_{config.suffix}.pt")
